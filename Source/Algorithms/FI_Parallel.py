...
class FastInterchange:
        ...
        def evaluateDestinationsForSwap(self, dests, i, goin, return_dict):
                fixedCost = 0
                changeCausedByRemoving = {}
                for d in dests:
                        ...
                        if newDistance < currentDistance:
                                ...
                                if changeCausedByRemoving.get(mySource) is None:
                                        changeCausedByRemoving[mySource] = 0 
                ...
                return_dict[i] = (fixedCost,changeCausedByRemoving)             
                return return_dict

        def myMoveEvalParallel(self, goin):
                goOut = -1
                changeCausedByRemoving = {} #source's destid to cost
                #netChangeIn
                w = sys.maxsize
                changeCausedByRemoving = {}
                combinedChange = {}
                processDestDictionary = {}
                manager = multiprocessing.Manager()
                numOfProcesses = 5
                return_dict = manager.dict()
                for d in self.destinations:
                        procId = d%numOfProcesses
                        if processDestDictionary.get(procId) is None:
                            processDestDictionary[procId] = {}
                        processDestDictionary[procId][d] = self.destinations.get(d)
                procs = []
                if __name__ ==__name__:
                        for i in range(numOfProcesses):
                                p = multiprocessing.Process(target=self.evaluateDestinationsForSwap, args=(processDestDictionary[i],i,goin,return_dict))
                                procs.append(p)
                                p.start()
                ...
                fixedCost = 0   
                changeCausedByRemoving = {} #source's destid to cost
                for k, tup in return_dict.items():
                        fixedCost += tup[0]
                        for source in tup[1]:
                                if changeCausedByRemoving.get(source) is None:
                                        changeCausedByRemoving[source] = 0
                                changeCausedByRemoving[source] += tup[1].get(source)
                #print(str(changeCausedByRemoving))
                #print("FCost" + str(fixedCost))
                #print("Length of change dict " +  str(len(changeCausedByRemoving)))
        ...
        def myMoveEval(self, goin):
                ...
                #print("Move eval " + str(goin)  + ": " + str(move))
                return move
        def updateDestinationDistances(self, dests, i,goin_min, gout_min, return_dict):
                distances = {}
                for d in dests:
                        if self.closestSources[d] == gout_min:
                                if self.util.calculateDistanceBetweenDestinations(self.destinations.get(d), self.destinations.get(goin_min)) < self.util.calculateDistanceBetweenDestinations(self.destinations.get(d), self.destinations.get(self.secondClosestSource[d])):
                                ...
                                if self.util.calculateDistanceBetweenDestinations(self.destinations.get(d), self.destinations.get(self.closestSources[d]) ) > self.util.calculateDistanceBetweenDestinations(self.destinations.get(d), self.destinations.get(goin_min)):
                                        ...
                                        self.secondClosestSource[d] = self.pickSecondMinSource(d)       
                        distances[d] = {'closest':self.closestSources[d],'secondClosest':self.secondClosestSource[d]}
                return_dict[i] = distances
                return return_dict
        def updateClosestAndSecondClostestParallel(self, goin_min, gout_min):
                processDestDictionary = {}
                manager = multiprocessing.Manager()
                numOfProcesses = 5
                return_dict = manager.dict()
                ...
                if __name__ ==__name__:
                        for i in range(numOfProcesses):
                                p = multiprocessing.Process(target=self.updateDestinationDistances, args=(processDestDictionary[i],i,goin_min, gout_min,return_dict))
                                procs.append(p)
                                p.start()
                #print("starting process " + str(p.pid) + " num of assigned sources " + str(len(processSourceDictionary[i])))
                #p.join()
                ...
                for k,distances in return_dict.items():
                        for d,dis in distances.items():
                                self.closestSources[d] = dis['closest']
                                self.currentDestinationToSourceMapping[d] = dis['closest']
                                self.secondClosestSource[d] = dis['secondClosest']
                for d in self.closestSources:
                ...
                while not terminationCondition:
                        ...
                        if wmin >=0:
                                ...
                                if not parallelize:
                                        ...
                                        #self.updateClosestAndSecondClostest(goin_min, gout_min)
                                        self.updateClosestAndSecondClostestParallel(goin_min, gout_min)
        ...
        def initializeCurrentSources(self, sources):
                pm.em.destinations  = copy.deepcopy(dests)
